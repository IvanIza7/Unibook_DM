...
public class EditarActividad extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            actividadId = getArguments().getInt("actividad_id");
            actividadActual = db.getActividad(actividadId);
        }
        ...
        view.findViewById(R.id.btnGuardarActividad).setOnClickListener(v -> guardarCambiosActividad());
    ...
    private void cargarDatosActividad() {
        if (actividadActual != null) {
            edtTitulo.setText(actividadActual.getNombre());
            edtDescripcion.setText(actividadActual.getDescripcion());
            edtFecha.setText(actividadActual.getFechaEntrega());
            seekBarAvance.setProgress(actividadActual.getPorcentaje());

            if (actividadActual.getMateriaNombre() != null && !actividadActual.getMateriaNombre().isEmpty()) {
                for (int i = 0; i < listaNombresMaterias.size(); i++) {
                    if (listaNombresMaterias.get(i).equals(actividadActual.getMateriaNombre())) {
    ...
    private void guardarCambiosActividad() {
        ...
        int resultado = db.actualizarActividad(
                actividadId,
                titulo,
                descripcion,
